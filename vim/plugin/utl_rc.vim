...
elseif has("unix")
    ...
    "	Samples for other browsers are welcome!
    " [ew]:
    "let g:utl_rc_app_browser = ':n %u'
    "let g:utl_rc_app_browser = ":!w3m '%u'"
    "let g:utl_rc_app_browser = ":sil exe \"!w3m '%u'\" | redraw!"
    "let g:utl_rc_app_browser = ":ene | sil exe \"0r !w3m -dump -cols 80 '%u' 2> /dev/null\" | sil 1,$s/\\v\\n{3,}/\\r\\r/ | 1 | set nomod bh=hide"
    "let g:utl_rc_app_browser = ":ene | sil exe \"0r !w3m -dump -cols 80 '%u' 2> /dev/null | cat -s\" | sil d | sil 1,$s/\\s\\+$// | 1 | set nomod bh=hide ft=txt"
    "let g:utl_rc_app_browser = ":ene | sil exe \"0r !ew3dump '%u'\" | sil d | sil 1 | set nomod bh=hide ft=txt"
    let g:utl_rc_app_browser = ":ene | sil exe \"0r !ew3dump '%u'\" | sil d | sil 1 | file %u | set nomod ro bh=hide ft=txt"
...
elseif has('unix')
    ...
    " [ew]:
    "let g:utl_rc_app_mailer = "!mutt '%u'" 
    "let g:utl_rc_app_mailer = "sil !mutt '%u'" 
    let g:utl_rc_app_mailer = "sil exe \"!mutt '%u'\" | redraw!" 
...
elseif has("unix")
    ...
    " [ew]:
    "let g:utl_mt_text_html = ':n %p'
    "let g:utl_mt_text_html = ":!w3m '%p'"
    "let g:utl_mt_text_html = ":sil exe \"!w3m '%p'\" | redraw!"
    "let g:utl_mt_text_html = ":ene | sil exe \"0r !w3m -dump -cols 80 '%p' 2> /dev/null | cat -s\" | sil d | sil 1,$s/\\s\\+$// | 1 | set nomod bh=hide ft=txt"
    "let g:utl_mt_text_html = ":ene | sil exe \"0r !ew3dump '%p'\" | sil d | sil 1 | set nomod bh=hide ft=txt"
    "let g:utl_mt_text_html = ":ene | sil exe \"0r !ew3dump '%p'\" | sil d | sil 1 | file %p | set nomod bh=hide ft=txt"
    "let g:utl_mt_text_html = ":n %p | se noro | 1,$d | sil exe \"0r !ew3dump %\" | sil d | sil 1 | set nomod ro nowrite bh=hide ft=txt"
    let g:utl_mt_text_html = ":n %p | se noro | 1,$d | sil exe \"0r !ew3dump %\" | sil d | sil 1 | set nomod ro bh=hide ft=txt"
