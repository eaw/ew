...
" date(s): 2002-12-14..2002-12-15..2002-12-19
"
" description:
"
...
function! Mcline(arg)  " {
  ...
  " x: "exactly" -- match line exactly
  if a:arg ==# 'x'
    let rx = escape(getline('.'), '\/')             " slash and backslash
    let rx = '\V\^' . rx . '\$'

  " s: "space" -- match line with relaxed treatment of whitespace (leading and
  "               trailing ws is optional, the number of consecutive inner
  "               ws-characters are ignored) (useful when the headings in the
  "               toc are indented (relative to the main text))
  elseif a:arg ==# 's'
  ...
  " d: "dot/digit" -- match line ignoring dots after digits (useful for text
  "                   files generated from sgml, where the heading numbering
  "                   often has trailing dots in the main text but not in the
  "                   toc (eg linux howtos))
  elseif a:arg ==# 'd'
  ...
  " a: "anywhere" -- match line anywhere
  elseif a:arg ==# 'a'
    let rx = escape(getline('.'), '\/')             " slash & backslash
  ...
  " A: "anywhere/approximately" -- match line anywhere and approximately,
  "                                ignoring dots after digits (this is meant to
  "                                be the most "forgiving" way of calling the
  "                                function, so it might change later)
  elseif a:arg ==# 'A'
    let rx = escape(getline('.'), '\/')             " slash & backslash
    let rx = substitute(rx, '^\s\+', '', '')        " del leading whitespace
    let rx = substitute(rx, '\s\+$', '', '')        " del trailing whitespace
